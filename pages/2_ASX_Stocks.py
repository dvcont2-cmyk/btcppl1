...
def composite_score(row):
    ...
    if pd.isna(rsi):
        ...
        indicators.append(("RSI (14)", 2, "B", f"Strongly oversold at {rsi:.1f} - Strong Buy signal (<30)"))
    elif rsi < 40:
        indicators.append(("RSI (14)", 1, "B", f"Oversold at {rsi:.1f} - Buy zone (<40)"))
    elif rsi > 70:
        indicators.append(("RSI (14)", -1, "S", f"Overbought at {rsi:.1f} - Sell signal (>70)"))
    else:
        indicators.append(("RSI (14)", 0, "N", f"Neutral at {rsi:.1f} - No clear signal (40-70)"))

    k = row["StochRSI_k"]
    if pd.isna(k):
        indicators.append(("Stoch RSI", 0, "N", "Neutral - not enough data"))
    elif k < 20:
        indicators.append(("Stoch RSI", 1, "B", f"Oversold at {k:.1f} - reversal up likely (<20)"))
    elif k > 80:
        indicators.append(("Stoch RSI", -1, "S", f"Overbought at {k:.1f} - reversal down likely (>80)"))
    else:
        indicators.append(("Stoch RSI", 0, "N", f"Neutral at {k:.1f} - between 20 and 80"))
    ...
    if pd.isna(bb_pct):
        ...
        indicators.append(("Bollinger Bands", 1, "B", "Price below lower band - oversold Buy signal"))
    elif close > bb_upper:
        indicators.append(("Bollinger Bands", -1, "S", "Price above upper band - overbought Sell signal"))
    elif bb_pct < 0.2:
        indicators.append(("Bollinger Bands", 1, "B", f"Price near lower band ({bb_pct*100:.0f}%) - leaning oversold"))
    elif bb_pct > 0.8:
        indicators.append(("Bollinger Bands", -1, "S", f"Price near upper band ({bb_pct*100:.0f}%) - leaning overbought"))
    else:
        indicators.append(("Bollinger Bands", 0, "N", f"Price mid-band ({bb_pct*100:.0f}%) - neutral"))

    ema200 = row["EMA_200"]
    if pd.isna(ema200):
        indicators.append(("EMA 200 Trend", 0, "N", "Neutral - not enough data for 200-period EMA"))
    elif close > ema200:
        indicators.append(("EMA 200 Trend", 2, "B", f"Price above EMA 200 ({ema200:,.4f}) - long-term uptrend"))
    else:
        indicators.append(("EMA 200 Trend", -2, "S", f"Price below EMA 200 ({ema200:,.4f}) - long-term downtrend"))

    ema50 = row["EMA_50"]
    if pd.isna(ema50) or pd.isna(ema200):
        indicators.append(("EMA 50/200 Cross", 0, "N", "Neutral - not enough data"))
    elif ema50 > ema200:
        indicators.append(("EMA 50/200 Cross", 2, "B", "Golden Cross - EMA 50 above EMA 200 - bullish long-term"))
    else:
        indicators.append(("EMA 50/200 Cross", -2, "S", "Death Cross - EMA 50 below EMA 200 - bearish long-term"))

    adx, adx_pos, adx_neg = row["ADX"], row["ADX_pos"], row["ADX_neg"]
    if pd.isna(adx):
        indicators.append(("ADX Strength", 0, "N", "Neutral - not enough data"))
    elif adx < 20:
        indicators.append(("ADX Strength", 0, "N", f"ADX {adx:.1f} - weak trend, signals less reliable"))
    elif adx_pos > adx_neg:
        indicators.append(("ADX Strength", 1, "B", f"ADX {adx:.1f} - strong trend, bullish direction (+DI > -DI)"))
    else:
        indicators.append(("ADX Strength", -1, "S", f"ADX {adx:.1f} - strong trend, bearish direction (-DI > +DI)"))

    cci = row["CCI"]
    if pd.isna(cci):
        indicators.append(("CCI (20)", 0, "N", "Neutral - not enough data"))
    elif cci < -100:
        indicators.append(("CCI (20)", 1, "B", f"CCI {cci:.0f} - oversold below -100, potential Buy signal"))
    elif cci > 100:
        indicators.append(("CCI (20)", -1, "S", f"CCI {cci:.0f} - overbought above +100, potential Sell signal"))
    else:
        indicators.append(("CCI (20)", 0, "N", f"CCI {cci:.0f} - neutral between -100 and +100"))

    wr = row["WilliamsR"]
    if pd.isna(wr):
        indicators.append(("Williams %R", 0, "N", "Neutral - not enough data"))
    elif wr < -80:
        indicators.append(("Williams %R", 1, "B", f"Williams %R {wr:.1f} - oversold below -80, Buy zone"))
    elif wr > -20:
        indicators.append(("Williams %R", -1, "S", f"Williams %R {wr:.1f} - overbought above -20, Sell zone"))
    else:
        indicators.append(("Williams %R", 0, "N", f"Williams %R {wr:.1f} - neutral between -80 and -20"))
    ...
    if pd.isna(roc):
        ...
        indicators.append(("ROC (12)", 0, "N", f"ROC {roc:.1f}% - momentum flat (-5% to +5%)"))
...
def swing_trade_commentary(row, df, ticker, price, score, info):
    ...
    if golden_cross and above_ema200 and rsi_oversold and macd_bullish:
        ...
        signals.append(("SWING BUY - Pullback in Uptrend",
            f"Golden Cross active + price above EMA 200. RSI ({rsi:.1f}) has pulled back to oversold while MACD histogram just turned positive - classic dip-buy setup in a healthy uptrend.",
        ...
        signals.append(("SWING WATCH - Extreme Oversold Reversal",
            f"Triple oversold confluence: RSI ({rsi:.1f}) below 30, Williams %R ({wr:.1f}) below -80, Stoch RSI K ({stoch_k:.1f}) below 20. High probability of at least a short-term bounce. " +
            ("Caution - Death Cross still in effect, keep size small." if death_cross else "No Death Cross - more reliable signal."),
            f"Entry: ${price:,.4f} | Stop: ${stop:,.4f} (-{((price-stop)/price)*100:.1f}%) | Target: ${target:,.4f} (+{((target-price)/price)*100:.1f}%) | R:R {rr:.1f}x",
            "yellow"))
        ...
        signals.append(("SWING BUY - Momentum Breakout",
            f"TTM Squeeze just fired. ADX ({adx:.1f}) confirms strong trend with +DI ({adx_pos:.1f}) above -DI ({adx_neg:.1f}). MACD confirming. Trail stop rather than fixed target.",
            f"Entry: ${price:,.4f} | Stop: ${stop:,.4f} (-{((price-stop)/price)*100:.1f}%) | First Target: ${target:,.4f} (+{((target-price)/price)*100:.1f}%) | R:R {rr:.1f}x",
            "green"))
    elif death_cross and rsi_overbought and stoch_high:
        setup = "overbought_short"
        signals.append(("CAUTION - Overbought in Downtrend",
            f"Death Cross active + RSI ({rsi:.1f}) overbought + Stoch RSI K ({stoch_k:.1f}) above 80. High risk of reversal down. Not a long entry. Existing holders consider reducing.",
        ...
        if score >= 0:
            signals.append(("NO CLEAR SWING SETUP - Leaning Neutral/Bullish",
                "Indicators are mixed or consolidating. No high-conviction entry signal. " +
                ("Golden Cross provides a positive backdrop - wait for a dip to EMA 50 or RSI < 45." if golden_cross else "Watch for Golden Cross or oversold bounce to form a setup."),
                "Monitor for RSI dip below 40 or TTM Squeeze firing to trigger entry.",
                "gray"))
        else:
            signals.append(("NO CLEAR SWING SETUP - Leaning Bearish",
                "More bearish signals than bullish but no clean reversal setup yet. " +
                ("Death Cross active - downtrend bias." if death_cross else "Trend weakening.") +
                " Avoid new longs until RSI < 30 and MACD turns positive.",
    ...
    if week52_high and week52_low and week52_high != week52_low:
        ...
        signals.append(("52-Week Range Context",
            ...
            ("Near lows - potential value zone for swing entries." if range_pos < 30 else
             "Near highs - higher risk for new entries, tighter stops needed." if range_pos > 70 else
             "Mid-range - no particular edge from price position alone."),
            "blue"))

    signals.append(("ATR Volatility - Position Sizing",
        f"ATR (14): ${atr:,.4f} | Suggested stop: ${atr*1.5:,.4f} (1.5x ATR) | ${atr*2:,.4f} (2x ATR)",
...
if is_swing_suitable:
    ...
    st.info("ETF / Low-liquidity stock - TA signals apply but better suited to DCA / long-term holding than active swing trading.")
...
for title, detail, levels, color_key in swing_signals:
    ...
    st.markdown(
        f"<div style='border-left: 4px solid {color}; padding: 10px 16px; margin-bottom: 12px; background: rgba(255,255,255,0.03); border-radius: 4px;'>"
...
with row1_l:
    st.markdown(
        f"<h4>RSI + Stoch RSI {val_span('RSI: ' + rsi_val, '#F59E0B')} {val_span('K: ' + stoch_val, '#60A5FA')} {val_span('D: ' + stochd_val, '#F472B6')}</h4>",
        unsafe_allow_html=True)
    fig1 = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.5, 0.5], subplot_titles=("RSI (14)", "Stochastic RSI"))
    fig1.add_trace(go.Scatter(x=df["time"], y=df["RSI"], name="RSI", line=dict(color="#F59E0B", width=2)), row=1, col=1)
    ...
    fig1.add_trace(go.Scatter(x=df["time"], y=df["StochRSI_k"], name="Stoch K", line=dict(color="#60A5FA", width=2)), row=2, col=1)
    fig1.add_trace(go.Scatter(x=df["time"], y=df["StochRSI_d"], name="Stoch D", line=dict(color="#F472B6", width=1, dash="dot")), row=2, col=1)
...
with row1_r:
    st.markdown(
        f"<h4>Price + BB + EMA {val_span('BB: ' + bb_pct_val, 'gray')} {val_span('EMA50: ' + ema50_val, '#34D399')} {val_span('EMA200: ' + ema200_val, '#F87171')}</h4>",
        unsafe_allow_html=True)
    fig2 = go.Figure()
    fig2.add_trace(go.Candlestick(x=df["time"], open=df["open"], high=df["high"], low=df["low"], close=df["close"], name="Price"))
    fig2.add_trace(go.Scatter(x=df["time"], y=df["BB_upper"], name="BB Upper", line=dict(color="gray", dash="dot", width=1)))
    fig2.add_trace(go.Scatter(x=df["time"], y=df["BB_lower"], name="BB Lower", line=dict(color="gray", dash="dot", width=1), fill="tonexty", fillcolor="rgba(128,128,128,0.1)"))
    fig2.add_trace(go.Scatter(x=df["time"], y=df["BB_mid"],   name="BB Mid",   line=dict(color="gray", width=1)))
    fig2.add_trace(go.Scatter(x=df["time"], y=df["EMA_50"],   name="EMA 50",   line=dict(color="#34D399", width=1.5)))
    fig2.add_trace(go.Scatter(x=df["time"], y=df["EMA_200"],  name="EMA 200",  line=dict(color="#F87171", width=1.5)))
    support_s, resistance_s = detect_support_resistance(df)
    for s in support_s:
        fig2.add_hline(y=s, line_dash="dot", line_color="lime", opacity=0.5, annotation_text=f"S ${s:,.4f}", annotation_position="bottom left")
    for r in resistance_s:
        fig2.add_hline(y=r, line_dash="dot", line_color="tomato", opacity=0.5, annotation_text=f"R ${r:,.4f}", annotation_position="top left")
...
with row2_l:
    st.markdown(
        f"<h4>MACD {val_span('MACD: ' + macd_val, '#60A5FA')} {val_span('Sig: ' + msig_val, '#F472B6')} {val_span('Hist: ' + mhst_val, 'gray')}</h4>",
        unsafe_allow_html=True)
    fig3 = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.45, 0.55], subplot_titles=("Price", "MACD"))
    fig3.add_trace(go.Scatter(x=df["time"], y=df["close"], name="Price", line=dict(color="#F59E0B")), row=1, col=1)
    fig3.add_trace(go.Scatter(x=df["time"], y=df["MACD"], name="MACD", line=dict(color="#60A5FA", width=2)), row=2, col=1)
    fig3.add_trace(go.Scatter(x=df["time"], y=df["MACD_signal"], name="Signal", line=dict(color="#F472B6", width=1.5)), row=2, col=1)
    colors_macd = ["green" if v >= 0 else "red" for v in df["MACD_hist"].fillna(0)]
    fig3.add_trace(go.Bar(x=df["time"], y=df["MACD_hist"], name="Histogram", marker_color=colors_macd), row=2, col=1)
...
with row2_r:
    st.markdown(
        f"<h4>ADX - Trend Strength {val_span('ADX: ' + adx_val, '#A78BFA')} {val_span('+DI: ' + dip_val, '#34D399')} {val_span('-DI: ' + din_val, '#F87171')}</h4>",
        unsafe_allow_html=True)
    fig_adx = go.Figure()
    ...
    fig_adx.add_hline(y=25, line_dash="dash", line_color="white", annotation_text="Trend threshold")
...
with row3_l:
    st.markdown(f"<h4>CCI {val_span('CCI: ' + cci_val, '#F59E0B')}</h4>", unsafe_allow_html=True)
    fig_cci = go.Figure()
...
with row3_r:
    st.markdown(f"<h4>Williams %R {val_span('%R: ' + wr_val, '#60A5FA')}</h4>", unsafe_allow_html=True)
    fig_wr = go.Figure()
...
with row4_l:
    st.markdown(f"<h4>ROC - Rate of Change {val_span('ROC: ' + roc_val, '#F59E0B')}</h4>", unsafe_allow_html=True)
...
with row4_r:
    st.markdown(f"<h4>ATR - Volatility {val_span('ATR: ' + atr_val, '#C084FC')}</h4>", unsafe_allow_html=True)
    fig_atr = go.Figure()
...
with row5_l:
    st.markdown(f"<h4>TTM Squeeze {val_span('Squeeze: ' + sq_val, sq_color)}</h4>", unsafe_allow_html=True)
    ...
    fig_sq.add_trace(go.Scatter(x=df["time"], y=[0]*len(df), mode="markers",
        marker=dict(color=dot_colors, size=6, symbol="circle"), name="Squeeze dot"))
...
with row5_r:
    ...
    if "volume" in df.columns:
        ...
        fig_vol    = go.Figure()
        fig_vol.add_trace(go.Bar(x=df["time"], y=df["volume"], name="Volume", marker_color=vol_colors, opacity=0.7))
        fig_vol.add_trace(go.Scatter(x=df["time"], y=avg_vol, name="20-period Avg", line=dict(color="#F59E0B", width=1.5, dash="dot")))
